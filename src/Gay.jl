...
# Include Drand - League of Entropy distributed randomness beacon
include("drand.jl")
using .Drand
export DrandBeacon, DrandRound, fetch_latest, fetch_round, round_at_time
export drand_seed, to_seed, verify_round, quicknet, mainnet, cross_verify
...
# Content-addressable seeding via SHA-256 (for bytes/files)
function _to_seed(x::AbstractVector{UInt8})
    bytes = sha256(x)
    return reinterpret(UInt64, bytes[1:8])[1]
end
_to_seed(io::IO) = _to_seed(read(io))

# Original seed phrase variant (String/Symbol/etc) - FNV-1a on hash()
function _to_seed(x)
...
- Bytes: `gay_seed(read("file.bin"))` (SHA-256 content-addressable)
- IO: `gay_seed(open("file.bin"))` (SHA-256 content-addressable)
"""
gay_seed(n::Integer) = gay_seed!(n)
gay_seed(x) = gay_seed!(_to_seed(x))

# ═══════════════════════════════════════════════════════════════════════════
# Unified Seeding Interface with drand Integration
# ═══════════════════════════════════════════════════════════════════════════

"""
    gay_seed!(x)
    gay_seed!(:record)       # Random, Will Be Known (returns DrandRound)
    gay_seed!(:record, id)   # Random, Known (replay recorded round)
    gay_seed!(:new)          # Random, Unknown (drand, discarded)
    gay_seed!(:offline)      # Random, Unknown, Local (system entropy)

Unified seeding interface with 6 strategies:

1. **Known (Deterministic)**: Provide a seed phrase/number
   ```julia
   gay_seed!("experiment_v1")  # String seed phrase
   gay_seed!(42)               # Integer seed
   gay_seed!(read("data.bin")) # SHA-256 of file contents
   ```

2. **Random, Known (Replay)**: Replay a recorded drand round
   ```julia
   gay_seed!(:record, 12345678)  # Fetch specific round
   ```

3. **Random, Will Be Known (Live, Recorded)**: Fetch fresh drand, return proof
   ```julia
   dr = gay_seed!(:record)  # Returns DrandRound
   # Save dr.round for later replay
   ```

4. **Random, Unknown (Live, Unrecorded)**: Fetch fresh drand, discard proof
   ```julia
   gay_seed!(:new)  # Uses League of Entropy quicknet (3s rounds)
   ```

5. **Random, Unknown, Local (Offline)**: System entropy
   ```julia
   gay_seed!(:offline)  # Fast, no network, local RandomDevice
   ```

6. **Content-Addressable**: SHA-256 of byte content
   ```julia
   gay_seed!(read("config.json"))  # Same file = same seed
...
# Command: :record (Strategy 3 - Will Be Known)
function gay_seed!(::Val{:record})
    seed, dr = Drand.drand_seed(quicknet())
    GLOBAL_GAY_RNG[] = GayRNG(seed)
    return dr  # Return proof for later replay
end

# Command: :record round_id (Strategy 2 - Known Replay)
function gay_seed!(::Val{:record}, round_id::Int)
    seed, dr = Drand.drand_seed(round_id, quicknet())
    GLOBAL_GAY_RNG[] = GayRNG(seed)
    return dr
end

# Command: :new (Strategy 4 - Unknown, uses drand)
function gay_seed!(::Val{:new})
    seed, _ = Drand.drand_seed(quicknet())
    GLOBAL_GAY_RNG[] = GayRNG(seed)
    return nothing  # Proof intentionally discarded
end

# Command: :offline (Strategy 5 - Local system entropy)
function gay_seed!(::Val{:offline})
    # Use RandomDevice for cryptographic-quality local entropy
    seed = rand(RandomDevice(), UInt64)
    GLOBAL_GAY_RNG[] = GayRNG(seed)
    return seed
end

# Dispatch to Val for symbol commands
function gay_seed!(s::Symbol)
    if s in (:record, :new, :offline)
        return gay_seed!(Val(s))
    else
        # Treat as deterministic seed (hash the symbol)
        return invoke(gay_seed!, Tuple{Any}, s)
    end
end

# Two-argument dispatch for :record round_id
function gay_seed!(s::Symbol, round_id::Int)
    if s == :record
        return gay_seed!(Val(:record), round_id)
    else
        error("Unknown command: gay_seed!(:$s, $round_id)")
    end
end
